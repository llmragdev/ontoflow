...
scope: "타입 정본, 영속 지식, Apply 경계, 단일 검증기, 일괄 등록, 바인딩, 실행 상태, 근거, 대화 입력 계약, Gate 확장 계약의 구현 기준"
...
# 워크플로우·온톨로지 통합 거버넌스 — 상세 설계 r3

> 구현자가 테이블과 계약으로 바로 읽는 문서다.
> 제품 방향과 컴포넌트 경계는 개괄 설계 문서에서 다룬다.
> 확정된 계약은 §1–§18, §20에, 남은 판단은 §19에 있다.
...
○ 확장      프로토타입 + 계약 고정. 상용화는 후속
```

### 1.2 계약을 지금 고정하는 이유

```text
Gate 를 나중에 붙일 때 실행 상태 구조를 다시 깨지 않기 위해
대화 입력을 나중에 붙일 때 슬롯 스키마를 다시 깨지 않기 위해
```
...
정의 테이블의 유일 키는 `api_name`으로 잡는다. `display_name`이 바뀌어도 참조가 깨지지 않아야 한다.
...
```text
project_id = "__base__"   전역 기본 정본. 읽기 전용
project_id = <프로젝트>    해당 프로젝트의 정의
```

| 규칙 | 내용 |
|---|---|
| 상속 | 프로젝트는 전역 정본을 상속한다 |
...
**호출 주소와 인증 정보를 타입 정본에 직접 넣지 않는다.**

| 이유 | 설명 |
|---|---|
| 보안 | 정본 쓰기 권한이 곧 임의 주소 호출 권한이 되는 것을 막는다 |
...
슬롯 유형을 별도 정본으로 두지 않는다. 영속 지식과 연결되는 슬롯은 `(object_type, property_definition)`에서 파생된다.
...
| `ontology_change_history` | 정본 또는 영속 인스턴스가 **실제로** 바뀐 기록 | Ontology Manager |
...
`slot_scope`는 **정본 결속 여부**, `validation_basis`는 **실제 검증 경로**다.

| 검증 경로 | `slot_scope` | `validation_basis` | `schema_version_id` | 검증 내용 |
|---|---|---|:---:|---|
| 정본 검증 | `bound` | `tbox_schema` | **필수** | 타입·범위·열거·필수·참조·카디널리티 |
| 런타임 검증 | `transient` | `runtime_type` | null | 값 타입 일치만 |
| 우회 | `bound`/`transient` **유지** | `bypassed` | null | 검증 안 함. 사유 필수 |

**우회는 슬롯의 종류를 바꾸지 않는다.** 원래 `slot_scope`를 유지해야 재검증 배치가 "정본 검증을 받아야 했던 슬롯"을 찾을 수 있다.
...
ABox도 사용자가 `atomic`을 선택할 수 있다. **TBox는 `partial`을 선택할 수 없다** — 정의 일부만 적용되면 정본이 불완전한 상태로 남는다.

### 6.7 재적용 안전성

```text
dry_run = true 로 검증한 뒤 같은 source_payload_ref 로 적용한다
검증 시점과 적용 시점 사이에 정본이 바뀌면 재검증한다
...
| 상태 | 조건 | 처리 |
|---|---|---|
| `validated` | 정본 존재 + 검증 통과 | 승격 가능 |
| `pending_type` | 정본 없음, 제안 대기 | **값 보관 가능, 승격 불가** |
| `invalid` | 정본 존재 + 검증 실패 | 승격 불가 |

`pending_type`은 초기 설계 마찰을 줄이는 완충 장치일 뿐 정본 우회로가 아니다. **실행 단위별 비율 상한**을 둔다(D-6).
...
**단 자유 문자열 키가 아니라 정본과 역할에 결속된 참조만 허용한다.**

### 8.2 공용 상태 패턴의 위험과 대응

| 위험 | 대응 |
|---|---|
| 이름 충돌 | 슬롯 이름을 타입 정본에 결속(§12.1) |
...
| 13 | 참조하는 정본 정의의 `status`가 `RETIRED`가 아닌가 | 저장 거부 |
...
**서로 대체하지 않는다.** "문서에 있다"와 "정본으로 승인되었다"는 다른 말이다.

| 하나만 있을 때 | 위험 |
|---|---|
| 문서 근거만 | 유사 문장은 찾았으나 대상 객체와 연결되는 경로가 없다 |
| 경로 근거만 | 관계는 있으나 왜 정본인지 설명할 수 없다 |
...
7. 참조한 정본 정의의 status 가 RETIRED 가 아니다               (§3.2)
...
**후보값은 검증되지 않았다.** 정본 결속도 역할 바인딩도 없다. **이관 시점에 비로소 Unified Validator를 통과한다.**
...
> **프로토타입 범위는 `DRAFT` · `SUBMITTED` · `APPROVED` · `REJECTED`다.**
> `COMMITTED` · `COMMIT_FAILED` · `CONFLICTED`는 **계약만 고정**하고 상용화 시 구현한다.
...
```text
타입 정본 조회 계층    →  슬롯 결속, 정적 검사, 검증 기준
영속 지식 저장소       →  Apply 경계의 대상
Apply 경계            →  영속 지식 저장소 + 변경 이력
Unified Validator     →  타입 정본 + 영속 지식
일괄 등록             →  Validator + Apply 경계
정본 변경·영향 분석    →  Validator (가상 적용 모드) + Apply 경계
워크플로우 정의·제어 선  →  실행 경로 기록의 전제
바인딩 계층           →  타입 정본 + 워크플로우 정의
역할 바인딩           →  실행 단위 + 역할 선언
...
| 단계 | 내용 | 이 문서의 절 |
|:---:|---|---|
| **Core-1** | 타입 정본 기본 구조 | §3.1 ~ §3.6, §3.10 |
...
항목 번호는 `D-N`으로 고정한다. 추가·삭제 시에도 기존 번호를 재사용하지 않는다.

| # | 판단 | 선택지 | 범위 | 시점 |
|---|---|---|:---:|---|
| **D-1** | 전역 정본의 버전 승격 | 프로젝트가 명시적으로 올림 / 자동 추종 | ◆ | Core-1 |
...
```text
타입 정본 결속은 쓰기 경로마다 조회를 늘린다.
슬롯 하나를 검증할 때 정본 조회 · 근거 연결 · 버전 확인 · 바인딩 확인이 붙는다.
...
> `State Slot`, `WriteBack`, `Gate`, `ObjectType` 등 일부는 상용 데이터·온톨로지 플랫폼의 어휘와
> 겹쳐 보일 수 있으나, **특정 제품의 공식 용어를 인용하거나 그 정의를 따르지 않는다.**

특히 `State Slot`은 널리 통용되는 표준 용어가 아니다. 상용 플랫폼에서는 이 계층의 역할이 액션 파라미터·객체 속성값·제출 조건·실행 컨텍스트·감사 로그로 **나뉘어** 있는 경우가 많다. 한 계층으로 모으는 이유는 흩어져 있으면 "실행 중 값"과 "조직 지식"의 경계가 지점마다 다르게 그어지기 때문이다.

### 20.2 용어표

| 용어 | 의미 | 범위 |
|---|---|:---:|
| 타입 정본 | 승인된 객체·속성·관계·액션·워크플로우 정의 | ◆ |
| 공유 속성 | 여러 객체 유형이 함께 쓰는 속성 정의 | ◆ |
| 영속 지식 | 객체 인스턴스·속성값·관계 인스턴스 | ◆ |
| Apply 경계 | 영속 지식을 바꾸는 단일 진입점 | ◆ |
| Unified Validator | 모든 경로가 공유하는 단일 검증기 | ◆ |
| 변경 제안 | 타입 정본을 바꾸기 위한 승인 대상 | ◆ |
| 변경 이력 | 정본·영속 지식이 실제로 바뀐 기록 | ◆ |
| 일괄 등록 | 파일·API로 들어오는 대량 정의·데이터 | ◆ |
| 부작용 수준 | 액션이 외부·정본에 미치는 영향의 등급 | ◆ |
| 역할 | 실행 안에서 대상 객체가 맡는 이름 | ◆ |
| 역할 바인딩 | 역할을 실제 인스턴스에 묶는 실행 상태 | ◆ |
| 역할 한정 슬롯 참조 | `{object_role, slot_name}` | ◆ |
| 상태 슬롯 | 실행 중 값의 격리 저장 단위 | ◆ |
| 제어 선 | 실행 순서를 잇는 선 | ◆ |
| 바인딩 계층 | 절차가 정본을 참조하는 지점 | ◆ |
...
```text
1. 코어는 온톨로지 매니저와 워크플로우 매니저다. 상용 수준으로 만든다.
2. 대화 입력은 앞단 부품, Gate 는 뒷단 부품, 자동화는 측면 부품이다.
3. 부품은 구현을 미루되 계약은 지금 고정한다.
...
```text
 6. 타입 정본은 객체·속성·공유속성·관계·액션·워크플로우 유형이다.
 7. 전역 정본은 예약 프로젝트에 담고, 프로젝트는 상속·확장만 하며 약화할 수 없다.
...
34. Gate 프로토타입 상태는 DRAFT, SUBMITTED, APPROVED, REJECTED 다.
35. COMMITTED, COMMIT_FAILED, CONFLICTED 는 계약만 고정하고 상용화 시 구현한다.
